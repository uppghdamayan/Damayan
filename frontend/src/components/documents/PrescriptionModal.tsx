...
export function PrescriptionModal({ patientId, visitId, onClose }: PrescriptionModalProps) {
  const { data: draft, isLoading, error } = useDocumentDraft(patientId, 'PRESCRIPTION', visitId);
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-150">
      <div className="px-[18px] py-5 flex flex-col gap-4 overflow-y-auto">
        <DocumentDraftPreview draft={draft} title="PRESCRIPTION (Draft)" />
