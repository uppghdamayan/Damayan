...
export function Sidebar() {
  ...
  // Close sidebar on Escape key when open as overlay
  useEffect(() => {
  ...
  const handleSelect = (p: Patient) => {
    setActivePatient(p);
    // Close overlay drawer if viewport is tablet/mobile (<1280px)
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-semibold text-text-primary truncate max-[1439px]:max-w-[130px]">
                      {p.lastName}, {p.firstName}
                    </div>
                    <div className="font-mono text-[10px] text-text-muted truncate mt-0.5">
                      {sexLabel} · {age} yrs · <span className="max-[1439px]:hidden">#{p.patientCode}</span>
                    </div>
                  </div>
  ...
  return (
    <>
      {/* Inline Sidebar (>=1280px) */}
      <aside
        suppressHydrationWarning
        className={cn(
          "bg-surface flex-col h-full shrink-0 overflow-hidden transition-[width] duration-300 ease-in-out border-r border-border",
      ...
      <aside
        className={cn(
          "fixed top-[var(--topbar-h)] left-0 bottom-0 z-[310] bg-surface border-r border-border overflow-y-auto flex flex-col",
          "w-[var(--sidebar-w)] transition-transform duration-200 ease-out min-[1280px]:hidden",
